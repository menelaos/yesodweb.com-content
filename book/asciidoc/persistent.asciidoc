...
instance PersistEntity Person where
    newtype Key Person = PersonKey {unPersonKey :: (BackendKey SqlBackend)}
...
|===============
|Function|Returns
|selectSource|A +ConduitT+ containing all the IDs and values from the database. This allows you to write streaming code.

NOTE: A +ConduitT+ is a monad transformer designed for streaming data, and is part of the +conduit+ package. I
...
|selectKeys|Returns only the keys, without the values, as a +ConduitT+.
|===============
...
properly interpret it. This also allows us to use values containing white space,
so if we want to set someone's default home country to be El Salvador:
...
main = runSqlite ":memory:" $ do
    ...
    runConduit $ rawQuery sql [] .| mapM_C (liftIO . print)
----
...
[NOTE]
====
The type of +runDB+ is +YesodDB site a -> HandlerFor site a+. +YesodDB+ is defined as:
...
For more information on the syntax of entity declarations, database connection,
etc., check out https://github.com/yesodweb/persistent/tree/master/docs.
