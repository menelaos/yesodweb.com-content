...
storing and retrieving information for the user's active session. It also
supplies a source of random numbers for token generation and a helper function
for efficiently updating the "Expires" header. In real Yesod applications, as
you'll see shortly, you don't need to interact with these values directly, but
it's informative to understand what's under the surface.
...
features. Let's address that, starting with the +HandlerFor+ monad.
...
To encapsulate all of this common functionality, Yesod provides the +HandlerFor+
monad. The vast majority of the code you write in Yesod will live in this monad,
so you should get acquainted with it. Let's start off by replacing our previous
+YesodDispatch+ instance with a new one that takes advantage of +HandlerFor+:
...
import           Network.Wai.Handler.Warp (run)
import qualified Text.Blaze.Html5         as H
import           Yesod.Core               (HandlerFor, Html, RenderRoute (..),
...
the route constructor name.) Notice its signature: +HandlerFor App Html+. It's
so common to have the monad +HandlerFor App+ that most applications have
a type synonym for it, +type Handler = HandlerFor App+. The function is
...
+HandlerFor+ monads into WAI ++Application++s. Let's look at its type signature:
...
(Route site)+ and +YesodRunerEnv site+. To get our +HandlerFor site res+
...
+MonadResource+, and so can automatically take advantage of those
functionalities.
...
parameters: the content type of the response, and a +ConduitT+ that produces a stream of +Flush
Builder+. Yesod also provides a number of convenience functions for creating
that +ConduitT+, such as +sendChunk+, +sendChunkBS+, and +sendChunkText+.
...
import           Control.Monad                      (forM_)
import           Network.Wai                        (pathInfo)
import           Yesod.Core                         (HandlerFor, RenderRoute (..),
                                                     TypedContent, Yesod,
...
Now that we've finished our detour into the details of the +HandlerFor+
monad, let's get back to higher-level Yesod request processing. So far,
