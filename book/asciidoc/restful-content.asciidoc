...
+PATCH+:: Updates the resource partially on the server. When you want to update
one or more fields of the resource, this method should be preferred.

+DELETE+:: Just like it sounds: wipe out a resource on the server. Calling
multiple times should be OK.

To a certain extent, this fits in very well with Haskell's philosophy: a +GET+
...
link:https://hackage.haskell.org/package/aeson[the Haddock documentation]
