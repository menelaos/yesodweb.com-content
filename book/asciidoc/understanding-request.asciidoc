...
on top of that which you are used to dealing with: +HandlerFor+. When you write
...
+runHandler+ function will turn a +HandlerFor+ into a +YesodApp+. +yesodRunner+ takes this
...
+Request+, convert it to a +YesodRequest+, construct a +RunHandlerEnv+, hand that
